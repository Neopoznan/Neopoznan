...
3. Скопируйте `config.example.yaml` в `config.yaml` и укажите:
   ...
   - `response_text` — что отправлять (по умолчанию «я поеду»); отправляется
     не больше одного раза за сообщение, даже если в нём несколько ключевых
     слов сразу;
   - `delay_seconds` — задержка перед отправкой после срабатывания (по
     умолчанию 3 секунды);
   - `cooldown_seconds` — минимальный интервал между двумя срабатываниями
     (по умолчанию 36000, то есть 10 часов);
